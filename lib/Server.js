    ...
    Config.Load(function(config) {
        ...
        if(config.Server.AutoStart) {
            ...
            self.IsRunning(function(res) {
                if(!res.running) {
                    // Not running, let's check for updates and start it.
                    self.CheckUpdate(function(status) {
                        if(!status) self.Start(function(game) {
            ...
            Steam.UpdateAvailable(function(update) {
                if(update.status) {
                    ...
                    if(config.Server.AutoUpdate.OnlyMajor && update.type == "major") {
                        ...
                        doUpdate = true;
                        Logger.log('debug', "[Update] Major patch (v"+update.available+"), running upgrade.");
                    } else if(!config.Server.AutoUpdate.OnlyMajor) {
                        // Update everything
                        doUpdate = true;
                        Logger.log('debug', "[Update] Patch (v"+update.available+") available, running upgrade.");
                    }

                    if(doUpdate) {
                        self.StopNice("Patch (v"+update.available+") available, running upgrade.", function() {
                            Steam.Update(function(u) {
