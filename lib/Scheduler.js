...
exports.Init = function(server, sio) {
    ...
    Config.Load(function (config) {
        ...
        timers.Update = new cronjob('0 */3 * * * *', function(){
                Server.IsRunning(function (game) {
                    if (global.timers.niceStop.length == 0 && !global.state.WaitForUpdate) {
        ...
        // Chat scheduler
        timers.Chat = new cronjob('*/5 * * * * *', function() {
            Logger.log('info', 'fetches chat');
            RCON.Command('getchat', function(res) {
                if(res.message) {
