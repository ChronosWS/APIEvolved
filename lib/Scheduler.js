    ...
    Config.Load(function (config) {
        ...
        timers.QueryData = later.setInterval(function() {
            Query.Run(function(data) {
               if(!data) {
                   global.QueryData = {status: false, server: null};
               } else {
                   global.QueryData = {status: true, server: data};
               }
            });
        }, querydata_scheduler);
        ...
        Object.keys(config.Server.Scheduler).forEach(function (key) {
            config.Server.Scheduler[key].forEach(function (task) {
                var timer = {
                    ...
                    action: key,
                    time: task.time,
                    schedule: later.parse.text('at ' + task.time),
...
exports.GetSchedule = function(callback) {
    ...
    timers.Scheduler.forEach(function(task) {
        schedule.push({
            id: task.id,
            action: task.action,
            next: later.schedule(task.schedule).next(3)[0],
            upcoming: later.schedule(task.schedule).next(3),
